...
public class LoginStateIntercepter implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(token==null) {
			this.renderJson(response, JsonData.buildCustom(-2,null,"非法访问"));
			return false;
		}
		...
		//检测token是否为空
		if(tokenValue==null) {
			this.renderJson(response, JsonData.buildCustom(-3, null, "请重新登陆"));
			jedis.close();
			return false;
		}
		//检测有效时间（>=0或为-1）
		...
		if(ttl<-1) {
			this.renderJson(response, JsonData.buildCustom(-4, null, "登陆超时"));
			jedis.close();
			return false;
		}
